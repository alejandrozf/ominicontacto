...
class PaginaMetaFacebook(models.Model):
    ...
    def get_stream_name(self):
        return 'facebook_meta_webhook_page_{}'.format(self.app_id)
...
class ConversationMessengerMetaApp(models.Model):
    """Modelo que representa una conversación entre un usuario y
        una página de Facebook a través de Messenger de Meta."""
    page = models.ForeignKey(PaginaMetaFacebook, on_delete=models.CASCADE,
    ...
    last_message = models.ForeignKey('MessageMessengerMetaApp', null=True, blank=True,
    ...
    def __str__(self):
        return f"Conversation between {self.page_client_id} and Page ID: {self.page.page_id}"
...
class MessageMessengerMetaApp(models.Model):
    ...
    conversation = models.ForeignKey(
        ConversationMessengerMetaApp, on_delete=models.CASCADE, related_name='messages', null=True)
    timestamp = models.DateTimeField(default=timezone.now, db_index=True)
    ...
    def __str__(self):
        ...
        return f"{direction} - {self.content.get('text', '')[:30]}(Message ID: {self.message_id})"
...
class AttachmentMessengerMetaApp(models.Model):
    """Modelo que representa un archivo adjunto en un mensaje de Messenger de Meta."""
    message = models.ForeignKey(MessageMessengerMetaApp, on_delete=models.CASCADE,
...
class MenuInteractivoMessengerMetaApp(models.Model):
    ...
    page = models.ForeignKey(
        PaginaMetaFacebook, related_name="menuinteractivo", on_delete=models.CASCADE,
...
class OpcionMenuInteractivoMessengerMetaApp(models.Model):
    opcion = models.OneToOneField(
        'configuracion_telefonia_app.OpcionDestino', on_delete=models.CASCADE,
        related_name="opcion_menu_messenger_meta_app")
