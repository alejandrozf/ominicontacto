...
export default {
    ...
    async agtWhatsCoversationSendWhatsappTemplateMessage (
        { commit },
        { conversationId = null, templateId = null, params_header, params, params_buttons, phoneLine, messages, $t }
    ) {
        try {
            ...
            const result = await service.sendWhatsappTemplateMessage(
                conversationId,
                { template_id: templateId, params_header, params, params_buttons}
            );
            const { status, data } = result;
            if (status === HTTP_STATUS.SUCCESS) {
                const itsMine = data.origin === phoneLine;
