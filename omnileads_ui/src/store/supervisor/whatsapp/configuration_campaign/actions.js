...
import Service from '@/services/supervisor/whatsapp/configuration_campaign_service';
const service = new Service();
const PARAMS_REGEX = /{{\d+}}/g;

export default {
    async initSupCampaignTemplates ({ commit }, campaignId) {
        try {
            const { status, data } = await service.campaignTemplatesInit(
                campaignId
            );
            let templates = [];
