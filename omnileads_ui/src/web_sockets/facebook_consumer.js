...
export class FacebookConsumer {
    ...
    static getInstance ({
        ...
        if (!FacebookConsumer.#instance) {
            FacebookConsumer.#instance = new FacebookConsumer(url);
        }
        return FacebookConsumer.#instance;
    }
    ...
    handleEventByType (type, args) {
        switch (type) {
        case FACEBOOK_EVENTS.NEW_MESSAGE:
            this.handleNewMessageEvent(args);
            break;
        case FACEBOOK_EVENTS.MESSAGE_STATUS:
            this.handleMessageStatusEvent(args);
            break;
        case FACEBOOK_EVENTS.NEW_CHAT:
            this.handleNewChatEvent(args);
            break;
        case FACEBOOK_EVENTS.CHAT_ATTENDED:
            this.handleChatAttendedEvent(args);
            break;
        case FACEBOOK_EVENTS.CHAT_TRANSFERED:
            this.handleChatTransferedEvent(args);
            break;
        case FACEBOOK_EVENTS.CHAT_EXPIRED:
    ...
    handleNewMessageEvent (data = null) {
        console.log('Facebook Consumer: New Message Event received');
        if (data) STORE.dispatch('agtFacebookConversationReciveMessage', data);
    }

    handleMessageStatusEvent (data = null) {
        if (data) STORE.dispatch('agtFacebookSendMessageStatus', data);
    }

    async handleNewChatEvent (data = null) {
        await notificationEvent(
            NOTIFICATION.TITLES.FACEBOOK_NEW_CHAT,
            'Nueva conversacion en espera de ser atendida',
            NOTIFICATION.ICONS.INFO
        );
        if (data) STORE.dispatch('agtFacebookReceiveNewChat', data);
    }

    async handleChatAttendedEvent (data) {
        await notificationEvent(
            NOTIFICATION.TITLES.FACEBOOK_CHAT_ATTENDED,
            `El chat de la campana (${data.campaign_name}), se atendio`,
            NOTIFICATION.ICONS.INFO
        );
        if (data) STORE.dispatch('agtFacebookChatsListInit', data);
    }

    async handleChatTransferedEvent (data) {
        await notificationEvent(
            NOTIFICATION.TITLES.FACEBOOK_CHAT_TRANSFERED,
