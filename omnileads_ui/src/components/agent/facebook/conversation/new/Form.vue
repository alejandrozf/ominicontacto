...
export default {
    ...
    methods: {
        ...mapActions(['initSupCampaignFacebookTemplates', 'agtFacebookContactSearch', 'agtFacebookContactDBFieldsInit']),
        ...
        updatedLocalStorage () {
            const resetForm = localStorage.getItem('agtFacebookConversationNewResetForm') === 'true';
            if (resetForm) {
                this.initializeData();
                localStorage.setItem('agtFacebookConversationNewResetForm', false);
            }
        },
        handleModal ({
            showModal = false,
            ...
            localStorage.setItem('agtFacebookConversationsList', true);
            if (response) {
        ...
        async getTemplatesByCampaign (campaignId) {
            const campaign = this.supFacebookPageCampaigns.find(
                (c) => c.id === campaignId
            );
            await this.initSupCampaignFacebookTemplates({
                campaignId,
    ...
    watch: {
        agtFacebookContactSearchResults: {
            handler () {
                this.contacts = this.agtFacebookContactSearchResults.map(
                    (contact) => {
                        return {
                            id: contact?.id || null,
        ...
        supCampaignFacebookTemplates: {
            handler () {
                if (this.supCampaignFacebookTemplates.length > 0) {
                    this.templates = this.supCampaignFacebookTemplates.filter(
