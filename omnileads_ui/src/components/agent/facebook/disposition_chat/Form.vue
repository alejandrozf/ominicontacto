...
import { FORM_TYPES, FIELD_TYPES, FIELD_TYPE_NUMBER } from '@/globals/agent/facebook/disposition';
...
export default {
    ...
    computed: {
        ...mapState([
            'agtFacebookDispositionChatOptions',
            'agtFacebookConversationInfo',
            'agtFacebookDispositionChatFormToCreate',
            'agtFacebookDispositionChatDetail'
        ])
    },
    methods: {
        ...mapActions([
            'agtFacebookDispositionChatUpdate',
            'agtFacebookDispositionChatCreate'
        ]),
        resetData () {
            this.form = {
                id: null,
        ...
        initFormData () {
            if (this.agtFacebookDispositionChatFormToCreate) {
        ...
        getFormFieldsByOption () {
            ...
            const option = this.agtFacebookDispositionChatOptions?.find(
        ...
        async save (isFormValid) {
            console.log('===> save disposition chat form');
            try {
                this.submitted = true;
                ...
                const formData = {
                    idContact: this.agtFacebookConversationInfo?.client?.id || null,
                    idAgente: this.agtFacebookConversationInfo?.agent || null,
                    ...
                    idConversation: this.agtFacebookConversationInfo.id
                };
                // Si es de GESTION hay que mandar respuestaFormularioGestion
                const dispositionType = this.agtFacebookDispositionChatOptions?.find(
                ...
                console.log('===> save disposition chat form', formData); 
                if (this.agtFacebookDispositionChatFormToCreate) {
                    response = await this.agtFacebookDispositionChatCreate({
                        data: formData
                    });
                } else {
                    response = await this.agtFacebookDispositionChatUpdate({
                        id: this.agtFacebookDispositionChatDetail.id,
                        data: formData
                    });
                }
                this.$swal.close();
                ...
                if (status === HTTP_STATUS.SUCCESS) {
                    const event = new Event(FACEBOOK_LOCALSTORAGE_EVENTS.CONVERSATION.DETAIL_INIT_DATA);
    ...
    watch: {
        ...
        agtFacebookDispositionChatOptions: {
            handler () {
                if (this.agtFacebookDispositionChatOptions.length > 0) {
                    const noAction =
            this.agtFacebookDispositionChatOptions.filter(
