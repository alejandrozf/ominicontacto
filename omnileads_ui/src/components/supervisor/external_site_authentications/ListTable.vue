<template>
  <div class="card">
    <DataTable
      :value="externalSiteAuthentications"
    ...
    >
      ...
      <Column
        field="nombre"
        :sortable="true"
        :header="$t('models.external_site_authentication.name')"
      ></Column>
      <Column
        field="url"
        :header="$t('models.external_site_authentication.url')"
      ></Column>
...
export default {
    ...
    methods: {
        ...
        detail (externalSiteAuthentication) {
            this.$emit('showDetail', externalSiteAuthentication);
        },
        edit (externalSiteAuthentication) {
            this.$router.push({
                name: 'supervisor_external_site_authentications_update',
                params: { id: externalSiteAuthentication.id }
            });
        },
        async remove (id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    ...
                    const response = await this.deleteExternalSiteAuthentication(id);
                    this.$swal.close();
                    const { status, message } = response;
                    if (status === HTTP_STATUS.SUCCESS) {
                        this.initExternalSiteAuthentications();
                        this.$swal(
                    ...
                    this.$swal(
                        this.$helpers.getToasConfig(
                            ...
                            this.$tc('globals.error_to_deleted_type', {
                                type: this.$tc('globals.external_site_authentication')
                            }),
                            this.$t('globals.icon_error')
                        )
                    );
        ...
        ...mapActions(['deleteExternalSiteAuthentication', 'initExternalSiteAuthentications'])
    },
    watch: {
        externalSiteAuthentications: {
            handler () {},
