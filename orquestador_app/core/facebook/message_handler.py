...
async def facebook_messenger_handler_messages(page, payloads):
    try:
        expire = None
        for entry in payloads['entry']:
            for messaging_event in entry['messaging']:
                timestamp = datetime.fromtimestamp(
                    int(messaging_event['timestamp']) / 1000,
                    timezone.get_current_timezone())
                print("messaging_event >>>>", messaging_event)
                context = None
                if 'message' in messaging_event:
                    ...
                    if 'text' in messaging_event['message']:
                        ...
                        file = attachment['payload'].get('url', None)
                    if 'quick_reply' in messaging_event['message']:
                        ...
                        if 'text' in messaging_event['message']:
                            content = {'text': messaging_event['message']['text']}
                        context = messaging_event['message']
                ...
                elif type in ['message', 'postback', 'quick_reply', 'image', 'video',
                              'audio', 'file', 'document', 'application']:
