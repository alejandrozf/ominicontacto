...
class CampanaTest(OMLBaseTest):
    ...
    def test_crear_agenda_contacto(self):
        ...
        base_datos = self.crear_base_datos_contacto()
        contacto = base_datos.contactos.first()
        hora = datetime.time(00, 00)
        fecha_actual= datetime.date.today()

        # crear agendacontacto para fecha anterior a la actual
        fecha_anterior = fecha_actual - datetime.timedelta(days=2)
        self.crear_agenda_contacto(agente, contacto, fecha_anterior, hora)

        # chequear que no devuelva anteriores a la fecha actual
        self.assertEqual(
            len(agente.agendacontacto.eventos_filtro_fecha('', '')), 0)

        # crear agendacontacto para fecha posterior a la actual
        fecha_posterior= fecha_actual + datetime.timedelta(days=2)
        self.crear_agenda_contacto(agente, contacto, fecha_posterior, hora)
