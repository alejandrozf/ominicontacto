...
class Command(BaseCommand):
    ...
    """
    help = u"Se regeneran las tareas cron configuradas para OMniLeads/Asterisk"

    def _regenerar_cronos(self):
        service = RegeneracionCronosService()
        service.regenerar_cronos()

    def handle(self, *args, **options):
        try:
            self._regenerar_cronos()
        except Exception as e:
            logger.error(f'Fallo del comando: {e}')
            raise CommandError(f'Fallo del comando: {e}')
