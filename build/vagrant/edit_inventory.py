...
parser.add_argument("--aio_build", help="Modifies if the install is selfhosted")
parser.add_argument("--release", help="Modifies if the install is selfhosted")
...
if args.aio_build == 'yes' and args.release:
    inventory_contents = inventory_contents.replace(
        "[build-aio]\n#localhost ansible_connection=local",
        "[build-aio]\nlocalhost ansible_connection=local")
    inventory_contents = inventory_contents.replace(
        "virtualenv_version=", "virtualenv_version={0}".format(args.release))
    inventory_file.seek(0)
    inventory_file.truncate()
    inventory_file.write(inventory_contents)
    sys.exit()
